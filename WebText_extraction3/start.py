...
# Check if psutil is installed, if not try to install it
try:
    # Try to import psutil directly if already installed
    import psutil
except ImportError:
    print("psutilライブラリがインストールされていません。インストールを試みます...")
    try:
        subprocess.check_call([sys.executable, "-m", "pip", "install", "psutil"])
        import psutil
        print("psutilのインストールに成功しました。")
    except Exception as e:
        print(f"エラー: psutilのインストールに失敗しました: {e}")
        print("手動で次のコマンドを実行してください: pip install psutil")
        sys.exit(1)
...
def run_script(script_name, *args):
    ...
    if not os.path.exists(script_name):
        print(f"エラー: 実行するスクリプト {script_name} が見つかりません。", file=sys.stderr)
...
def run_script_with_cpu_limit(script_path, cpu_percent_ratio):
    """
    ...
    print(f"実行中: {script_path} を {len(cpu_list)}/{num_cpus_total} CPU(s) を使用して実行 (指定CPU割合: {cpu_percent_ratio*100:.0f}%)")
    ...
    try:
        ...
        if process.returncode == 0:
            print(f"{os.path.basename(script_path)} の実行に成功しました。")
            return True
        else:
            print(f"{os.path.basename(script_path)} の実行中にエラーが発生しました。リターンコード: {process.returncode}", file=sys.stderr)
            return False
